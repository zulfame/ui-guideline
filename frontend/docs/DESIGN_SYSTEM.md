...
- **Sortable Header (konsisten):** SEMUA header yang bisa di-sort pakai satu komponen (`SortableHeader`) berbasis `Button variant="ghost"` dengan gaya **selaras teks TableHead** (`text-muted-foreground font-medium`, `-ml-2 h-8 px-2`) + ikon `ArrowUp/ArrowDown/ArrowUpDown` sesuai state. **Jangan** campur header tombol (foreground) dengan header teks polos (muted) — itu inkonsistensi (insiden Update 49).
