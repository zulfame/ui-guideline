...
**8. Card Spacing** — `CardHeader` `px-6 py-4 space-y-1.5` + `border-b` (pemisah header↔body); `CardContent` `px-6 py-4`; `CardFooter` `px-6 py-4` + `border-t` (pemisah body↔footer). Ritme vertikal `py-4` (16px) menjaga **compact**; horizontal `px-6` (24px). Antar-kartu di grid `gap-4`. Jangan bungkus padding ganda di dalam Card.
...
| Update 40 | **Input OTP full-width di Form Elements.** Slot OTP default `w-9` (lebar tetap) menyisakan ruang kosong di kartu. Di `FormElementsPage` saja: `InputOTP containerClassName="w-full"`, `InputOTPGroup className="w-full"`, tiap `InputOTPSlot className="flex-1"` → 6 slot melebar rata mengisi lebar kartu. Primitive `input-otp.jsx` **tidak diubah** (R38) sehingga OTP di Form Layout & dialog preview tetap lebar natural. Diverifikasi via screenshot. |
