...
| Application CMS — Roles/Jabatan | ✅ | **CRUD nyata** pohon `parent_id` (1 atasan langsung; rantai atasan otomatis): Card + DataTable tree-indent (chevron per depth), kolom Direct superior + Superiors chain `›`, dialog pemilih atasan **Combobox (searchable)** yang **mengecualikan diri+keturunan** (anti-siklus), Delete destructive (red) + promosi anak saat delete. Rute `/roles` | `pages/app/RolesPage.jsx` | Master data jabatan/role |
