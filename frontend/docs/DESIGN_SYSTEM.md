...
| Form Layout (sample forms) | ✅ | 2 grid berdasarkan ukuran: form kecil (**Login, Reset Password, OTP Verification**) di `md:grid-cols-2 lg:grid-cols-3` + form besar (**Register, Change Password, Contact**) di `md:grid-cols-2` (keduanya `items-start`). Di bawahnya 2 **section full-width**: **Multi-step Wizard** (StepIndicator + 3 langkah Account/Profile/Review, validasi per-langkah via `form.trigger`, Back/Next/Finish) & **Profile & Settings** (Avatar + Change photo, grid name/email, Role Select, Bio Textarea, preferensi Switch ber-border, footer Cancel/Save). Semua Card header/body/footer + `react-hook-form`+`zod` | `pages/layouts/FormLayoutPage.jsx` | Kumpulan sample form dasar + lanjutan |
