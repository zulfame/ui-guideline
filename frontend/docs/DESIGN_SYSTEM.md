...
| DataTable layout | ✅ | **Toolbar** (global search + Faceted Filter Role/Status + Density) · **Add** di `CardHeader` · Table dgn row-selection (Checkbox+select-all), **SortableHeader** full-cell (semua kolom, `aria-sort`), `thead` `bg-muted/50`, status Badge, **row actions** (DropdownMenu ber-ikon, tanpa label) · Empty state `No Data Available` · **Footer** rows-per-page (Select 8/16/24) + "{first}–{last} of {total}" + Page x/y + prev/next · **Density** (Compact default) persist `localStorage`. Dep `@tanstack/react-table` (R37). | `pages/layouts/DataTableLayoutPage.jsx` | Halaman list/tabel data (rujuk 2C.7) |
